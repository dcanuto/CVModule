function Jquad(x::Vector{Float64},beta::Vector{Float64},rho::Float64)
    J = zeros(10,10);

    J[1,1] = 1.;
    J[2,1] = rho.*x[1]./(x[2].^2);
    J[3,1] = 1.;
    J[4,1] = 1.;
    J[5,1] = 1.;
    J[6,1] = 1./x[2];

    J[2,2] = (0.5.*beta[1].*x[2].^-0.5 .- rho.*x[1].^2.*x[2].^-3);
    J[3,2] = J[2,2];
    J[4,2] = J[2,2];
    J[5,2] = J[2,2];
    J[6,2] = -x[1].*x[2].^-2 .+ sqrt.(0.5.*beta[1]./rho).*x[2].^-0.75;

    J[1,3] = -1.;
    J[2,3] = -rho.*x[3]./(x[4].^2);
    J[7,3] = 1./x[4];

    J[2,4] = (-0.5.*beta[2].*x[4].^-0.5 .+ rho.*x[3].^2.*x[4].^-3);
    J[7,4] = -x[3].*x[4].^-2 .- sqrt.(0.5.*beta[2]./rho).*x[4].^-0.75;

    J[1,5] = -1.;
    J[3,5] = -rho.*x[5]./(x[6].^2);
    J[8,5] = 1./x[6];

    J[3,6] = (-0.5.*beta[3].*x[6].^-0.5 .+ rho.*x[5].^2.*x[6].^-3);
    J[8,6] = -x[5].*x[6].^-2 .- sqrt.(0.5.*beta[3]./rho).*x[6].^-0.75;

    J[1,7] = -1.;
    J[4,7] = -rho.*x[7]./(x[8].^2);
    J[9,7] = 1./x[8];

    J[4,8] = (-0.5.*beta[4].*x[8].^-0.5 .+ rho.*x[7].^2.*x[8].^-3);
    J[9,8] = -x[7].*x[8].^-2 .- sqrt.(0.5.*beta[4]./rho).*x[8].^-0.75;

    J[1,9] = -1.;
    J[5,9] = -rho.*x[9]./(x[10].^2);
    J[10,9] = 1./x[10];

    J[5,10] = (-0.5.*beta[5].*x[10].^-0.5 .+ rho.*x[9].^2.*x[10].^-3);
    J[10,10] = -x[9].*x[10].^-2 .- sqrt.(0.5.*beta[5]./rho).*x[10].^-0.75;

    return J
end
